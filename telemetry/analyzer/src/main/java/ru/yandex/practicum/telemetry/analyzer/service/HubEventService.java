...
public class HubEventService {
    ...
    public void handle(HubEventAvro event) {
        ...
        if (payload instanceof DeviceAddedEventAvro added) addSensor(hubId, added);
            ...
            scenarios.findByHubIdAndName(hubId, removed.getName()).ifPresent(scenarios::delete);
    ...
    private void addSensor(String hubId, DeviceAddedEventAvro event) {
        sensors.findById(event.getId()).ifPresentOrElse(sensor -> {
            if (!sensor.getHubId().equals(hubId))
                throw new IllegalArgumentException("Датчик уже зарегистрирован в другом хабе: " + event.getId());
    ...
    private void removeSensor(String hubId, String sensorId) {
        sensors.findByIdAndHubId(sensorId, hubId).ifPresent(sensor -> {
            ...
            conditions.deleteAll(obsoleteConditions);
            actions.deleteAll(obsoleteActions);
    ...
    private void saveScenario(String hubId, ScenarioAddedEventAvro event) {
        ...
        event.getConditions().forEach(c -> ids.add(c.getSensorId()));
        event.getActions().forEach(a -> ids.add(a.getSensorId()));

        Map<String, Sensor> sensorMap = sensors.findAllById(ids).stream()
                .filter(s -> s.getHubId().equals(hubId))
                .collect(Collectors.toMap(Sensor::getId, s -> s));

        if (sensorMap.size() != ids.size())
            throw new IllegalArgumentException("Не все устройства сценария зарегистрированы в хабе " + hubId);
        ...
        scenario.replaceConditions(conditions);
        scenario.replaceActions(actions);
        scenarios.saveAndFlush(scenario);
        this.conditions.deleteAll(obsoleteConditions);
        this.actions.deleteAll(obsoleteActions);
    }

    private Integer conditionValue(Object value) {
        return value == null ? null : value instanceof Boolean b ? (b ? 1 : 0) : (Integer) value;
    }
}
