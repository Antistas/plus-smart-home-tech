...
public class HubEventProcessor implements Runnable {
    ...
    private final Consumer<String, HubEventAvro> consumer; private final HubEventService service; private final String topic; private final Duration timeout; private final AtomicBoolean running = new AtomicBoolean(true);
    public HubEventProcessor(@Qualifier("hubEventConsumer") Consumer<String, HubEventAvro> consumer, HubEventService service,
        ...
        this.consumer = consumer; this.service = service; this.topic = topic; this.timeout = timeout;
    }
    @Override public void run() {
        try { consumer.subscribe(List.of(topic)); while (running.get()) { ConsumerRecords<String, HubEventAvro> records = consumer.poll(timeout); for (ConsumerRecord<String, HubEventAvro> record : records) service.handle(record.value()); if (!records.isEmpty()) consumer.commitSync(); } }
        catch (WakeupException e) { if (running.get()) throw e; } catch (Exception e) { log.error("Ошибка обработки событий хабов", e); } finally { consumer.close(); }
    }
