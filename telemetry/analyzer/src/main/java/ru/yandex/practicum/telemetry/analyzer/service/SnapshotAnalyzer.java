...
public class SnapshotAnalyzer {
    ...
    public SnapshotAnalyzer(ScenarioRepository scenarios, @GrpcClient("hub-router") HubRouterControllerGrpc.HubRouterControllerBlockingStub hubRouterClient) {
        this.scenarios = scenarios; this.hubRouterClient = hubRouterClient;
    }
    ...
    private Integer extract(ConditionTypeAvro type, Object data) {
        return switch (type) {
            ...
            case TEMPERATURE -> data instanceof TemperatureSensorAvro d ? d.getTemperatureC() : data instanceof ClimateSensorAvro d ? d.getTemperatureC() : null;
    ...
    private void execute(SensorsSnapshotAvro snapshot, Scenario scenario, Sensor sensor, Action action) {
        ...
        DeviceActionRequest request = DeviceActionRequest.newBuilder().setHubId(snapshot.getHubId()).setScenarioName(scenario.getName())
                .setAction(actionBuilder).setTimestamp(Timestamp.newBuilder().setSeconds(instant.getEpochSecond()).setNanos(instant.getNano())).build();
